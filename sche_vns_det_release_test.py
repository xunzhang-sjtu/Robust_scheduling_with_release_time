...
def variableNeighborhoodDescent(solution,N,mu,sigma,r):
    ...
    while i < 1:
        ...
        neiborSolution2 = []
        for ind1 in range(len(neiborSolution)):
            sol_tem = neiborSolution[ind1]
            len_sol_aban = len(sol_abandoned)

            flag = 0
            for ind2 in range(len_sol_aban):
                if (sol_tem == sol_abandoned[ind2]).all():
                    flag = 1
                    break
            if flag == 0:
                neiborSolution2.append(sol_tem)


        obj_set, x_dict_set = compute_neiborhood_obj(neiborSolution2,N,mu,sigma,r)
        print('neigbor sol:',i,' iteration:',it,'opt obj:',np.min(obj_set),'len',len(neiborSolution2))
        it = it + 1
        ...
        for ind2 in range(len(x_dict_set)):
            if ind2 != index_min:
                sol_abandoned.append(x_dict_set[ind2])
