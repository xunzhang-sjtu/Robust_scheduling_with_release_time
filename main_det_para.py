...
n = 10 # num of jobs
delta_mu = 3 # control lb of mean processing time
...
if __name__ == '__main__':
    ...
    for it in range(iterations):
        ...
        temp2,p_bar,p_low = generate_Normal(mu_p,std_p,n,S_train+S_test,0.1,0.9)
        np.sum(np.abs(temp2 - temp))/(S_train+S_test)

        diff = pd.DataFrame(temp2 - temp)
        diff.mad()
        train_data = temp[:,0:S_train]
        test_data = temp[:,S_train:S_test]
        ...
        c_set = np.arange(0,1000,20)
        c_set[0] = 0.0


        # sol_rs_without_x = dro_det_models.det_release_time_scheduling_wass_without_x(n,r_mu,0.0001,S_train,train_data,p_bar,p_low)

        # c_set = np.arange(1,1.5,0.02)
        # c_set[0] = 1.00001
        # c_set = c_set * sol_wass['obj']

        # sol_rs = dro_models.det_release_time_scheduling_RS(n,r_mu,c_set[0],S_train,train_data,p_bar,p_low)
        ...
        for i in range(len(c_set)):
            ...
            rst_wass_list[ins,c] = x_seq = np.int32(np.round(sol['x_seq'])+1) 
        ...
        for i in range(len(c_set)):
            c = c_set[i]
            tft_wass[i] = out_sample.computeTotal_det_release(n,test_data,r_mu,S_test-S_train, rst_wass_list[ins,c_set[i]])
        ...
        full_path = project_path + '/data/ins='+str(it) + '/S_train='+str(S_train)+'/n='+str(n)+'delta_mu='+str(delta_mu)+'delta_r='+str(delta_r)+'delta_ep='+str(delta_ep)
