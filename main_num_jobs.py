...
if __name__ == '__main__':
    ...
    for n in n_set:
        ...
        p_mu_m = 10 + 0 * np.random.rand(n,instance); # mean values of processing time
        ...
        d_mu = 1 + d_cv * p_mu_m[:,0].sum() * np.random.rand(n)
        d_mu_m = copy.deepcopy(p_mu_m)
        for i in range(instance):
            d_mu_m[:,i] = d_mu
        ...
        for ins in range(instance):
            ...
            p_mu_estimate = np.mean(p_hat_dict_Normal[0],axis =1)
            p_sigma_estimate = np.std(p_hat_dict_Normal[0],axis =1)
            ...
            #     # import sche_vns_det_release_test as svdr
            #     # x_seq,opt_obj,time_gap = svdr.vns(n,p_mu_estimate,moment_2nd,r_mu_estimate)
            ...
            # cpu_time[ins]=[det_cpu_time,saa_cpu_time_Normal,saa_cpu_time_log,dro_cpu_time]
