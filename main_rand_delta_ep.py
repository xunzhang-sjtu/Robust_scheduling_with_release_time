...
def main_process(n,S_train,S_test,quan_low,quan_bar,mu_p,std_p,mu_r,std_r,delta_ep):
    para = {}
    para['mu_p'] = mu_p
    para['r_mu'] = mu_r
    para['mad_p'] = mad_p
...
project_path = 'D:/IJPR/rand/rand_cpu_time'
...
if __name__ == '__main__':
    ...
    for ins in range(1):
        ...
        for n in n_all:
            ...
            mad_r = r_random*mu_r*delta_ep
            std_r = np.sqrt(np.pi/2)*mad_r
            main_process(n,S_train,S_test,quan_low,quan_bar,mu_p,std_p,mu_r,std_r,delta_ep)



        # ************************ effect of release mad *******************
        # n_all = [10,15,20]
        # for n in n_all:
        #     mu_p = np.random.uniform(10*delta_mu,50,n)
        #     mad_p = np.random.uniform(0,delta_ep*mu_p)
        ...
        #     delta_ep_r_all = [1.5,2,2.5]
        #     r_random = np.random.uniform(0,1)
        #     for delta_ep_r in delta_ep_r_all:
        #         mad_r = r_random*mu_r*delta_ep_r
        #         std_r = np.sqrt(np.pi/2)*mad_r

        #         delta_mad_r = delta_ep_r
        #         main_process(n,S_train,S_test,quan_low,quan_bar,mu_p,std_p,mu_r,std_r,delta_ep)
